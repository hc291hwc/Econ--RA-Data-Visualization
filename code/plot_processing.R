...
#----------------
# new edition,   plots_compare_race(selected_groups_only)
#----------------
...
#----------------
# selected groups 
# only black, asian, white, hispanic
#----------------

selected_groups <- c('WHITE, NON-HISPANIC',
                     'BLACK OR AFRICAN AMERICAN, NON-HISPANIC',
                     'HISPANIC, RACE SPECIFIED',
                     'ASIAN'
                     )

get_selected_df <- function(df, selected_groups){
  df <- df %>% 
    filter(race %in% selected_groups) 
  
  df$race[df$race == 'WHITE, NON-HISPANIC'] = 'WHITE'
  df$race[df$race == 'BLACK OR AFRICAN AMERICAN, NON-HISPANIC'] = 'BLACK OR AFRICAN AMERICAN'
  df$race[df$race == 'HISPANIC, RACE SPECIFIED'] = 'HISPANIC'
  
  return(df)
}


df_math_plotting_for_grade <- get_selected_df(df_math_plotting_for_grade, selected_groups)
df_reading_plotting_for_grade <- get_selected_df(df_reading_plotting_for_grade, selected_groups)
...
#----------------------- 
plot_score_distribution_math_race_comp <- function(df_plotting, cur_grade, folder ){
  ...
  ggsave(filename =  glue('math_scores_comparison_between_race_{cur_grade}.pdf')  , path = glue('{folder}/math'), width = 10, height = 5.6)
...
#----------------------- 
plot_score_distribution_reading_race_comp <- function(df_plotting, cur_grade, folder ){
  ...
  ggsave(filename =  glue('reading_scores_comparison_between_race_{cur_grade}.pdf')  , path = glue('{folder}/reading'), width = 10, height = 5.6)
...
for (i in year_to_plot){
  plot_score_distribution_math_race_comp(df_math_plotting_for_grade, i, 'plots_compare_race(selected_groups_only)')
  plot_score_distribution_reading_race_comp(df_reading_plotting_for_grade, i, 'plots_compare_race(selected_groups_only)')
  }
